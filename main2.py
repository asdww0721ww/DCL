...
if __name__ == "__main__":
    ...
    with open(r'/home/linqx/Coding/3Dcanet/datalist/us_train_list2','r') as f:
    ...
    mcel3 = torch.nn.CrossEntropyLoss().to(device)
    mcel4 = torch.nn.CrossEntropyLoss().to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for MR_s_name, MR_s_imgs, MR_s_masks, CT_name, CT_imgs, CT_masks, MR_t_name, MR_t_imgs, MR_t_masks, US_name, US_imgs, US_masks in tqdm(dataset_train):
            ...
            #forward pass
            # mod: rewrite in & out
            a_out_all1, a_out_all2, seg_pred1, seg_pred2, a_out_all3, a_out_all4, seg_pred3, seg_pred4, a_out1,a_out2, a_out3, a_out4, z_out1, mu_out1, logvar_out1, z_out2, mu_out2, logvar_out2, x1_d, x2_d, x1t2_d, x2t1_d, x1_d, x2_d, x1t2_d, x2t1_d, a_out_all1_d,a_out_all2_d, a_out_all1t2_d, a_out_all2t1_d, z_out1_d, mu_out1_d, logvar_out1_d, z_out2_d, mu_out2_d, logvar_out2_d, z_out1t2_d, mu_out1t2_d, logvar_out1t2_d , z_out2t1_d, mu_out2t1_d, logvar_out2t1_d, loss1_mix, loss2_mix, loss1t2_mix,loss2t1_mix = model(MR_s_imgs, CT_imgs,MR_t_imgs, US_imgs,'training')
            
            #loss computation

            # cycle_consistency_loss
            cycle_consistency_loss_1 = L1Loss(
                x1_d.view(-1, MR_s_imgs.shape[-2], MR_s_imgs.shape[-1]),MR_s_imgs.view(-1, MR_s_imgs.shape[-2], MR_s_imgs.shape[-1]).float()
                )

            cycle_consistency_loss_2 = L1Loss(
                x2_d.view(-1, CT_imgs.shape[-2], CT_imgs.shape[-1]),CT_imgs.view(-1, CT_imgs.shape[-2], CT_imgs.shape[-1]).float()
                )
            ...
            gan_loss_1 = loss1_mix
            gan_loss_2 = loss2_mix
            ...
            #print(registered_seg1_1.shape)
            #print(target_seg1_1.shape)
            smooth = 1e-5
            ...
            seg_loss_1 = mcel3(seg_pred1,MR_s_masks.long())

            seg_loss_2 = mcel4(seg_pred2,CT_masks.long())

            # * total loss
            # mod : add temporal loss
            batch_loss = cycle_consistency_loss_1 + cycle_consistency_loss_2 + gan_loss_1 + gan_loss_2 + gan_loss_2_0 + gan_loss_1_0 + 20*seg_loss_1 + 20*seg_loss_2 + 20*seg_loss_3 + 20*seg_loss_4
