...
if __name__ == "__main__":
    ...
    mcel1 = torch.nn.CrossEntropyLoss().to(device)
    mcel2 = torch.nn.CrossEntropyLoss().to(device)
    mcel3 = torch.nn.CrossEntropyLoss().to(device)
    mcel4 = torch.nn.CrossEntropyLoss().to(device)
    mcel5 = torch.nn.CrossEntropyLoss().to(device)
    mcel6 = torch.nn.CrossEntropyLoss().to(device)
    mcel7 = torch.nn.CrossEntropyLoss().to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for MR_s_name, MR_s_imgs, MR_s_masks, CT_name, CT_imgs, CT_masks, MR_t_name, MR_t_imgs, MR_t_masks, US_name, US_imgs, US_masks in tqdm(dataset_train):
            ...
            # print(US_masks.shape)
            seg_loss_3 = mcel1(seg_pred3,MR_t_masks.long())

            seg_loss_4 = mcel2(seg_pred4,US_masks.long())
            ...
            batch_loss = 0.5*cycle_consistency_loss_1 + 0.5*cycle_consistency_loss_2 + 0.5*gan_loss_1 + 0.5*gan_loss_2 + 0.5*gan_loss_2_0 + 0.5*gan_loss_1_0 + 10*seg_loss_1 + 10*seg_loss_2 + 10*seg_loss_3 + 10*seg_loss_4
