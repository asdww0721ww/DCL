...
from models.ddpm.diffusion import GaussianDiffusionTrainer, GaussianDiffusionSampler
...
if __name__ == "__main__":
    ...
    ddpm_model = UNet(T=500,ch=32, ch_mult=[1, 1, 1, 1],attn=[2],num_res_blocks=1, dropout=0.1).to(device)
    ema_model = copy.deepcopy(ddpm_model).to(device)

    trainer = GaussianDiffusionTrainer(ddpm_model, 1e-4, 0.02, 500,).to(device)
    
        # self.net_sampler = GaussianDiffusionSampler(self.net_model, 1e-4, 0.02, 1000, self.h,'epsilon', 'fixedlarge')
        # self.net_sampler = GaussianDiffusionSampler(self.net_model, 1e-4, 0.02, 1000)
    net_sampler = GaussianDiffusionSampler(ddpm_model, 1e-4, 0.02, 500, 64, 'epsilon', 'fixedlarge').to(device)
    ema_sampler = GaussianDiffusionSampler(ema_model, 1e-4, 0.02, 500, 64, 'epsilon', 'fixedlarge').to(device)

    dimage1 = Discriminator(1).to(device)
    dimage2 = Discriminator(1).to(device)
    ...
    with open(r'/home/linqx/Coding/3Dcanet/datalist/us_train_list3','r') as f:
    ...
    for epoch in range(args.epochs):
        ...
        for MR_s_name, MR_s_imgs, MR_s_masks, CT_name, CT_imgs, CT_masks, MR_t_name, MR_t_imgs, MR_t_masks, US_name, US_imgs, US_masks in tqdm(dataset_train):
            ...
            #forward pass
            # mod: rewrite in & out
            # a_out_all1, a_out_all2, seg_pred1, seg_pred2, a_out_all3, a_out_all4, seg_pred3, seg_pred4, a_out1,a_out2, a_out3, a_out4, z_out1, mu_out1, logvar_out1, z_out2, mu_out2, logvar_out2, x1_d, x2_d, x1t2_d, x2t1_d, x1_d, x2_d, x1t2_d, x2t1_d, a_out_all1_d,a_out_all2_d, a_out_all1t2_d, a_out_all2t1_d, z_out1_d, mu_out1_d, logvar_out1_d, z_out2_d, mu_out2_d, logvar_out2_d, z_out1t2_d, mu_out1t2_d, logvar_out1t2_d , z_out2t1_d, mu_out2t1_d, logvar_out2t1_d, loss1_mix, loss2_mix, loss1t2_mix,loss2t1_mix = model(MR_s_imgs, CT_imgs,MR_t_imgs, US_imgs,MR_s_masks,CT_masks,MR_t_masks,US_masks,'training')

            a_out_all1, a_out_all2,seg_pred1, seg_pred2,a_out_all3, a_out_all4, seg_pred3, seg_pred4, z_out1, z_out2, a_out1 , a_out2 = model(MR_s_imgs, CT_imgs,MR_t_imgs, US_imgs,'training')
            ...
            '''
            # thi = random.randint(0,fake_x1.shape[2]-1)
            c_loss1_mix = []
            c_loss2_mix = []
            c_loss1t2_mix = []
            c_loss2t1_mix = []
            ...
            c_x1t2_d = []
            c_x2t1_d = []
            for thi in range(fake_x1.shape[2]):
                s_fake_x1 = fake_x1[:,:,thi,:,:]
                s_fake_x2 = fake_x2[:,:,thi,:,:]
                s_fake_x1t2 = fake_x1t2[:,:,thi,:,:]
                s_fake_x2t1 = fake_x2t1[:,:,thi,:,:]
            
                xt1_mix, h1_mix, loss1_mix = trainer(s_fake_x1)

                xt2_mix, h2_mix, loss2_mix = trainer(s_fake_x2)

                xt1t2_mix, h1t2_mix, loss1t2_mix = trainer(s_fake_x1t2)

                xt2t1_mix, h2t1_mix, loss2t1_mix = trainer(s_fake_x2t1)
                
                c_loss1_mix.append(loss1_mix)
                c_loss2_mix.append(loss2_mix)
                c_loss1t2_mix.append(loss1t2_mix)
                c_loss2t1_mix.append(loss2t1_mix)

                del xt1_mix, xt2_mix, xt1t2_mix, xt2t1_mix, s_fake_x1, s_fake_x2,s_fake_x1t2,s_fake_x2t1

                x1_d = ema_sampler(h1_mix)

                x1_d = torch.clamp(x1_d * 0.5 + 0.5,0,1)

                x2_d = ema_sampler(h2_mix)

                x2_d = torch.clamp(x2_d * 0.5 + 0.5,0,1)

                x1t2_d = ema_sampler(h1t2_mix)

                x1t2_d = torch.clamp(x1t2_d * 0.5 + 0.5,0,1)

                x2t1_d = ema_sampler(h2t1_mix)

                x2t1_d = torch.clamp(x2t1_d * 0.5 + 0.5,0,1)

                c_x1_d.append(x1_d)
                c_x2_d.append(x2_d)
                c_x1t2_d.append(x1t2_d)
                c_x2t1_d.append(x2t1_d)

                del x1_d, x2_d ,x1t2_d, x2t1_d

            loss1_mix = torch.cat(c_loss1_mix, dim=0)
            loss2_mix = torch.cat(c_loss2_mix, dim=0)
            loss1t2_mix = torch.cat(c_loss1t2_mix, dim=0)
            loss2t1_mix = torch.cat(c_loss2t1_mix, dim=0)

            loss1_mix = loss1_mix.mean()
            loss2_mix = loss2_mix.mean()
            loss1t2_mix = loss1t2_mix.mean()
            loss2t1_mix = loss2t1_mix.mean()
        
            x1_d = torch.cat(c_x1_d, dim=2)
            x2_d = torch.cat(c_x2_d, dim=2)
            x1t2_d = torch.cat(c_x1t2_d, dim=2)
            x2t1_d = torch.cat(c_x2t1_d, dim=2)

            del c_x1_d, c_x2_d, c_x1t2_d, c_x2t1_d 
            '''
            thi = random.randint(0,fake_x1.shape[2]-1)
            ...
            fake_x1t2[:,:,thi,:,:] = x1t2_d

            fake_x2t1[:,:,thi,:,:] = x2t1_d
            ...
            # a_out_all1_1, a_out_all2_1,seg_pred1_1, seg_pred2_1,a_out_all3_1, a_out_all4_1, seg_pred3_1, seg_pred4_1, z_out1_1, z_out2_1,a_out1_1, a_out2_1 = model(x1_d, x2_d,MR_t_imgs, US_imgs,MR_s_masks,CT_masks,MR_t_masks,US_masks,'training')


            # a_out_all1_2, a_out_all2_2,seg_pred1_2, seg_pred2_2,a_out_all3_2, a_out_all4_2, seg_pred3_2, seg_pred4_2, z_out1_2, z_out2_2, a_out1_2, a_out2_2 = model(x2t1_d, x1t2_d,MR_t_imgs, US_imgs,MR_s_masks,CT_masks,MR_t_masks,US_masks,'training')
            ...
            gan_loss_1 = loss1t2_mix
            gan_loss_2 = loss2t1_mix

            gan_loss_1_0 = loss1t2_mix
            gan_loss_2_0 = loss2t1_mix
            ...
            batch_loss =  gan_loss_1 + gan_loss_2 + gan_loss_2_0 + gan_loss_1_0 + 10*seg_loss_1 + 10*seg_loss_2 + 6*10*seg_loss_3 + 6*10*seg_loss_4

            # batch_loss = cycle_consistency_loss_1 + cycle_consistency_loss_2 + gan_loss_1 + gan_loss_2 + gan_loss_2_0 + gan_loss_1_0 + 10*seg_loss_1 + 10*seg_loss_2 + 6*10*seg_loss_3 + 6*10*seg_loss_4
            ...
            del seg_loss_3, seg_loss_4, a_out_all1, a_out_all2, seg_pred1, seg_pred2, a_out_all3, a_out_all4, seg_pred3, seg_pred4, a_out1,a_out2, a_out3, a_out4, z_out1, mu_out1, logvar_out1, z_out2, mu_out2, logvar_out2, x1_d, x2_d, x1t2_d, x2t1_d, x1_d, x2_d, x1t2_d, x2t1_d, a_out_all1_d,a_out_all2_d, a_out_all1t2_d, a_out_all2t1_d, z_out1_d, mu_out1_d, logvar_out1_d, z_out2_d, mu_out2_d, logvar_out2_d, z_out1t2_d, mu_out1t2_d, logvar_out1t2_d , z_out2t1_d, mu_out2t1_d, logvar_out2t1_d, loss1_mix, loss2_mix, loss1t2_mix,loss2t1_mix
